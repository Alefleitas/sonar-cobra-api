...
{
    public class NextCommunication : NotificationType
    {
        public override Notification Evaluate(List<DetalleDeuda> detallesDeuda, List<Debin> debins, List<Communication> communications, List<SsoUser> users, DateTime date, out Dictionary<string, List<int>> dataMapper)
        {
            ...
            var results = communications.Where(x => x.NextCommunicationDate != null && 
                x.NextCommunicationDate.Value.Date == date.Date).GroupBy(y => y.CommunicationCreatorUserId);
            ...
            foreach (var group in results)
            {
                ...
                var user = users.Where(x => x.IdApplicationUser == userId).FirstOrDefault();
                if (user != null) {
